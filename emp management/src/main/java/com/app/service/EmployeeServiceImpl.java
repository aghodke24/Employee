...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public String removeEmpDetails(Long empId) {
		Employee employee = empRepo.findById(empId)//rets empty optional in case of invalid emp id 
				//n rets populated emp holder in case of success.
	...
	public SigninResponse authenticate(SigninRequest request) {
		...
		//emp :src object(entity),Signingresponse.class:dest class type
		//entity--> dto
		return modelMapper.map(emp, SigninResponse.class);
	}

	@Override
	public SignupResponse authenticate(SignupRequest request) {
		Employee emp1 = modelMapper.map(request, Employee.class);
		empRepo.save(emp1);
//		Employee emp = empRepo.findByEmailAndPassword(request.getEmail(), request.getPassword())
//				.orElseThrow(() -> new ResourceNotFoundException("Bad Credentials , Invalid Login!!!!!!!!!!!!!"));
//		
		return modelMapper.map(emp1, SignupResponse.class);
	}
